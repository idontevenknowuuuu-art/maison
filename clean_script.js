...
async function placeOrder(){
  ...
  if(btn){ btn.disabled = true; btn.innerHTML = '⏳ Placing Order...'; btn.style.opacity = '0.6'; }
  ...
  // Clear cart immediately so user sees instant feedback
  saveCart([]);
  updateCartBadge();
  renderCart();
  closeCart();
  showToast('✅ Order #' + orderId.slice(-6) + ' placed! Sending confirmation...');
  
  // Save to Supabase & send emails in background
  try {
    const promises = [];
    ...
    if(supabaseClient) {
      promises.push(
        ...
        }).catch(e => console.warn('Could not save to Supabase:', e.message))
      );
    }
    ...
    promises.push(
      fetch('/api/order', {
        ...
        else showToast('📧 Confirmation emails sent!');
      }).catch(e => console.warn('Email send error:', e.message))
    );
    
    await Promise.all(promises);
  } catch(e) {
    console.warn('Order processing error:', e.message);
  }
